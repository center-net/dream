...
var x = <?php echo e(!empty($module_data) && $module_data->count_inputs > 0?
($module_data->count_inputs-1):0); ?>; //initlal text box count
$(add_button).click(function(e){ //on add input button click
e.preventDefault();
if(x < i){ //max input box allowed
	x++; //text box increment
	$(wrapper).append(<?php echo $__env->make('baboon.new_input', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>); //add input box
...
var x2 = <?php echo e(!empty($module_data) && $module_data->relation_count > 0?
($module_data->relation_count-1):0); ?>; //initlal text box count
$(add_button2).click(function(e){ //on add input button click
	e.preventDefault();
if(x2 < i2){ //max input box allowed
	x2++; //text box increment
	$(wrapper2).append(<?php echo $__env->make('baboon.relation_input', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>); //add input box
}
});
...
$(document).ready(function(){
  ...
  var fa_lists = <?php echo json_encode(fa()); ?>;
...
$(document).on('click','a.addcnamespace',function(e){
  var namespace = $('.c_namespace').val();
   ...
   $('.c_namespace').val('');
 $.ajax({
   url:'<?php echo e(url('it/create/namespace/controller')); ?>',
   dataType:'json',
   type:'post',
   data:{_token:'<?php echo e(csrf_token()); ?>',namespace:namespace}
 });
///// Append New Namespace Controller Folder ////////
...
// This for to put checkboxes to scan if checked or not //
    ...
    if(col_type_datatable[i] != 'dropzone'){
     table_checkbox += `
        ...
        `+col_name_datatable[i]+`
         <input type="checkbox" <?php echo e(!empty(request('module'))?'':'checked'); ?> class="dt_checkbox" name="dt_show_column[]" value="`+conv_col_checkbox[0]+`" />
        </label>
    </th>
    `;
    }
 ...
 <?php if(!empty(request('module')) && !empty(app('module_data')->datatable) && !empty(app('module_data')->datatable->dt_show_column)): ?>
 <?php
 $dt_show_columns = app('module_data')->datatable->dt_show_column;
 ?>
  <?php $__currentLoopData = $dt_show_columns; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $dt_show_column): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
   $('input[value="<?php echo e($dt_show_column); ?>"]').prop('checked', true);
...
var api_columns = '';
   for(i=0;i < col_name_api.length;i++){
    ...
    if(col_type_api[i] != 'dropzone'){
     api_columns += `
        <li>
        <label>
         <input type="checkbox" <?php echo e(!empty(request('module'))?'':'checked'); ?>  name="api_show_column[]" value="`+conv_col_checkbox[0]+`" />
        `+col_name_api[i]+`
        </label>
        </li>
    `;
  ...
  $('.api_columns_list').html(api_columns);
   <?php if(!empty(request('module')) && !empty(app('module_data')->api) && !empty(app('module_data')->api->api_show_column)): ?>
 <?php
 $api_show_columns = app('module_data')->api->api_show_column;
 ?>
  <?php $__currentLoopData = $api_show_columns; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $api_show_column): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
   $('input[value="<?php echo e($api_show_column); ?>"]').prop('checked', true);
  <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
 <?php endif; ?>
// Api Section End
...
<?php $__env->stopPush(); ?><?php /**PATH /home/helplgvr/public_html/dream/vendor/phpanonymous/it/it/views/baboon/bundel_js.blade.php ENDPATH**/ ?>